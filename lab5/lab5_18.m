...
imgR1 = interp2( X,Y,double(img),XR,YR,'linear'); % interpolacja obroconego
imgR2 = interp2( X,Y,double(img),XR,YR,'spline');
imgR3 = interp2( X,Y,double(img),XR,YR,'cubic');
imgR4 = interp2( X,Y,double(img),XR,YR,'nearest');
[Xi,Yi]=meshgrid( 1 : 0.2 : M, 1 : 0.2 : N ); % zageszczenie punktow
imgRi1 = interp2( X,Y,imgR1,Xi,Yi,'linear'); % interpolacja - poprawa jakosci
imgRi2 = interp2( X,Y,imgR2,Xi,Yi,'spline');
imgRi3 = interp2( X,Y,imgR3,Xi,Yi,'cubic'); 
imgRi4 = interp2( X,Y,imgR4,Xi,Yi,'nearest');

subplot(221); imshow( uint8(imgRi1)); title("Linear");
subplot(222); imshow( uint8(imgRi2)); title("Spline");
subplot(223); imshow( uint8(imgRi3)); title("Cubic");
subplot(224); imshow( uint8(imgRi4)); title("Nearest");
